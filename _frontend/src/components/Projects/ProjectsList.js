...
const ProjectsList = () => {
  ...
  // console.log("data projectlist -> ", projects);
  ...
  const handleDone = event => {
    ...
    let TMP_list = [];
    TMP_list = projects.projects.projects.filter(
    ...
    fetch(`${URL_HEROKU}move/finished-project/${id}`, {
      ...
      .then(data => {
        if (data.succes) {
          ...
          projects.dispatch({ type: DONE_PROJECTS, payload: TMP_list });
        }
      })
    ...
    await fetch(`${URL_HEROKU}delete/project/${id}`, {
      ...
      .then(data => {
        if (data.succes) {
          ...
          projects.dispatch({ type: DELETE_PROJECT, payload: TMP_list });
        }
      })
      .catch(err => console.log(err));
  };

  if (userLogged.userLogged.userInfo.succes) {
    return (
      <Wrapper>
        {renderRedirect(redirectTo)}

        {projects.projects.projects && projects.projects.projects.length > 0
          ? <div style={{ width: "100%", margin: "15px 0px" }}>
              {projects.projects.projects &&
                projects.projects.projects.map((project, key) =>
